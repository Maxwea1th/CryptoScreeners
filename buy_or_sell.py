...
def volatility_bot():
    ...
    open('list_tradingview.txt', 'w').close()
    ...
    for symbol in exchange.load_markets():
        ...
        df['overbought'] = rsi_indicator.rsi() >= 67.5
        df['oversold'] = rsi_indicator.rsi() <= 32.5
        ...
        if last_row['overbought'] == True:
            ...
            with open('list_tradingview.txt', 'a') as f:
    ...
    files = {'file': open('list_tradingview.txt', 'rb')}
    requests.post(WEBHOOK_VOLATILITY, files=files)
